...
!!  septic changes 1/28/09
	...
	if(ly1 == i_sep(j).and.isep_opt(j)==1) then
      ...
      !! restrict seepage if next layer is saturated !HR description is not correct: Limit seepage at bottom of soil profile
      if (ly1 == sol_nly(j)) then
        ...
        if (xx < 1.e-4) then
          ...
          !if (j==2) then
          !  write(*,*) subnum(j), hruno(j), xx, xx / (xx + Exp(8.833 - 2.598 * xx)) 
          !end if
          !if (xx / (xx + Exp(8.833 - 2.598 * xx))<0.75) then
          !   write(*,*) subnum(j), hruno(j), xx, xx / (xx + Exp(8.833 - 2.598 * xx))
          !endif
      ...
      !if (sepday + lyrtile + latlyr < sw_excess*0.95) then
      !  write(*,*) ly1, (sepday + lyrtile + latlyr) / sw_excess, 'waterbalance' !HR (this water is lost; mass balance issue?)
      !end if
      !if (ly1 == sol_nly(j)) then
      !  write(*,*) sw_excess, sepday+latlyr, sepday, latlyr 
      !endif
