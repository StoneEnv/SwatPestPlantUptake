...
!!in-stream processes
      if (rtwtr / 86400. > 1.e-6) then !HR Aug 2019 - Change threshold from 0.002 to avoid all pesticide going to benthic if low flow
        ...
        if (solpstin + sorpstin > 1.e-6) then
          if (chpst_koc(jrch) > 0.) then
            frsol = 1. / (1. + chpst_koc(jrch)*100 * sedcon)
          else
...
!!     &                                         chpstmass) * tday / depth
        ...
        !! calculate diffusion of pesticide between reach and sediment
        !!   based on difference in total between water column and 
        !!   benthic sediment concentration 
        difus = chpst_mix(jrch) * ((sedpstmass / sedpst_act(jrch)) - (chpstmass / depth)) * tday
...
!! calculate amount of pesticide transported out of reach
      if (rtwtr / 86400. > 1.e-6) then             !HR Aug 2019 - Change threshold from 0.002 to allow minor stream flow!Claire, corrected to match line 151
