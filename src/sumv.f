...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, k
      real*8 :: cnv
      real*8 :: totsolpst !total pesticide mass in soil profile
      ...
      !! watershed summations
        ...
        !! pesticide summary !HR ignore nyskip
        if (hrupest(j) == 1) then
          do k = 1, npmx
            !!HR calculate total pesticide in soil profile
            totsolpst = 0.
            do ly = 1, sol_nly(j)
                totsolpst = totsolpst + sol_pst(k,j,ly)
            end do
            !! HRU summary
            hrupstd(k,1,j) = pst_surq(k,j) * 1.e6 * hru_ha(j)
            hrupstd(k,2,j) = pst_sed(k,j) * 1.e6 * hru_ha(j)
            hrupstd(k,3,j) = (pst_surq(k,j) + pst_sed(k,j)) * 1.e6 * hru_ha(j) !HR added * hru_ha(j)
            hrupstd(k,4,j) = lat_pst(k) * 1.e6 * hru_ha(j)
            hrupstd(k,5,j) = tile_pst(k) * 1.e6 * hru_ha(j) !HR
            hrupstd(k,6,j) = pst_gw(k,j) * 1.e6 * hru_ha(j) !HR
            hrupstd(k,7,j) = pst_gwdeep(k,j) * 1.e6 * hru_ha(j) !HR
            hrupstd(k,8,j) = totsolpst * 1.e6 * hru_ha(j) !HR pesticide in entire soil profile
            hrupstd(k,9,j) = plt_pst2(k,j) * 1.e6 * hru_ha(j) !HR pesticide in plant
            ...
            wpstdayo(k,5) = wpstdayo(k,5) + tile_pst(k) * hru_dafr(j) !HR
            wpstdayo(k,6) = wpstdayo(k,6) + pst_gw(k,j) * hru_dafr(j) * 1.e6 !HR
            wpstdayo(k,7) = wpstdayo(k,7) + pst_gwdeep(k,j) * hru_dafr(j) * 1.e6 !HR
            wpstdayo(k,8) = wpstdayo(k,8) + totsolpst * hru_dafr(j) * 1.e6 !HR
            wpstdayo(k,9) = wpstdayo(k,9) + plt_pst2(k,j) * hru_dafr(j) * 1.e6  !HR
