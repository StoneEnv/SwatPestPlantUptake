...
!       itoth = mhruo
      ...
      open (32168,file="outputconc.pst",recl=600)
      iprintpsthrulayer = 0
      if (iprintpsthrulayer==1) then
      open (32137,file="tileoutput.pst",recl=600) !hendrik add temp pest output
      write(32137,"(26a11)") 'year','day','Sub','HRU','pestid','layer',
     &       'tilelayer',
     &      'sol_pst','pst_lat', 'pst_tile', 'pstsol',
     &      'qtile', 'flat', 'sol_prk','gw_q','sw_excess',
     &      'revapday','gwseep',
     &      'pstrm_frac', 'w_mobile', 'sat', 'sol_kp', 'sol_bd', 'layerthick',
     &      'mobPst_fr','tileC_ppb'
      end if
      iprintpstuptkhrulayer = 1
      if (iprintpstuptkhrulayer==1) then
      open (32138,file="pltuptk.pst",recl=600) !hendrik add temp pest output (plt uptake)
      write(32138,"(27a11)") 'year','day','Sub','HRU','pestid','layer',
     &       'tilelayer',
     &      'sol_pst','pst_lat', 'pst_tile', 'pst_perc',
     &      'qtile', 'flat', 'sol_prk','gw_q','pst_surq',
     &      'revapday','gwseep',
     &      'sol_pst_c', 'w_mobile', 'sat', 'sol_kp', 'reg_str', 'layerthick',
     &      'mobPst_fr', 'wuse', 'plt_upst'
      end if
...
!     icalen MUST be == zero  if IPRINT == 3 to print subdaily;
      ...
      read (101,*, iostat=eof) rtpestcust !HR use custom channel pesticide routing (rtepest2)
