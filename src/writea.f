...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &            wshdyro(40), wshdyro(43), wshdyro(41), wshdyro(111)
        ...
        if (iprint /= 1) then
          ...
          !!HR added lateral, tile, and gw, and soil and plant
          do j = 1, nhru
            if (hrupest(j) == 1) then
            sum = 0.
            do k = 1, npmx
     ...
     &                     (hrupsty(k,1,j), hrupsty(k,2,j),
     &                      hrupsty(k,4,j), hrupsty(k,5,j), 
     &                      hrupsty(k,6,j), hrupsty(k,7,j),
     &                      hrupsty(k,8,j), hrupsty(k,9,j), k = 1, npmx)
            end if
            end if
          end do
...
!         !! annual write--sediment routing (.sed)
 ...
 5100 format (1x,a5,a4,1x,i4,4x,1x,250(e17.4,1x))
