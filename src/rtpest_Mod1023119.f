...
!! initialize depth of water for pesticide calculations
      depth = 0.
      if (rchdep < 0.01) then !!MW_2019: Wonder why 0.1 m is set to the lowest depth??
        depth = 0.01
      else
        depth = rchdep
      endif
...
!      endif
      ...
      chpstmass = 0.
      !HR add previous timestep variable
      !!pstprev = chpst_conc(jrch) * rchwtr
      !!MW: New suggestion ... assume pstin is part of the beginning of timestep mass
      !pstprev = (chpst_conc(jrch) * rchwtr) + pstin
      ...
      if (chpstmass + sedpstmass < 1.e-9) then  !!MW_2019: Do we need to make the 1.e-6 lower??
        chpst_conc(jrch) = 0.
        sedpst_conc(jrch) = 0.
      end if
...
!!in-stream processes
      if (rtwtr / 86400. > 1.e-9) then !HR Aug 2019 - Change threshold from 0.002 to avoid all pesticide going to benthic if low flow
        !! calculated sediment concentration
        sedcon = 0.
        !sedcon = sedrch / rtwtr * 1.e6
        !!MFW, 10/15/15; The sed concentration is based on current initial sed (sedinorg) plus resuspended (rchdy(56,jrch); water volume is water stored + water in
        ...
        sedcon = (sedinorg) / (rchwtr + wtrin) * 1.e6  

        !! calculate fraction of soluble and sorbed pesticide
        frsol = 0.
        frsrb = 0.
        !if (solpstin + sorpstin > 1.e-6) then
        !! MFW, 10/16/15: This calculation of Frsol/Frsrb needs to happen every day based on the amount of sediment
        ...
        !if (frsrb>0) write(*,*) frsrb, chpst_koc(jrch), sedcon

        !! ASSUME POR=0.5; DENSITY=2.6E6; KD2=KD1
        !fd2 = 1. / (.5 + chpst_koc(jrch))
		!! MFW, 10/16/15: Corrected this equation basd on the manual
        ...
        !! calculate flow duration
         ...
         !!tday = 1.0 !!HR use actual routing time for routed elements
     ...
     &           * tday))
        ...
        ! HR August 2019; use full day for fraction in storage and actual travel time for routed elements
        !volatpst = chpst_vol(jrch) * frsol * chpstmass * tday / depth
        volatpst = chpst_vol(jrch) * frsol * chpstmass  / depth * ((tday * fr_routed) + (1. * fr_stored))   !!MW_2019: This makes sense to me
        ...
        !! settling
        ! HR Aug 2019; full day for in storage actual time routed
        !setlpst = chpst_stl(jrch) * frsrb * chpstmass * tday / depth
        setlpst = chpst_stl(jrch) * frsrb * chpstmass / depth * ((tday * fr_routed) + (1. * fr_stored))   !!MW_2019: This makes sense to me     
        !!HR Sep 2019 activate code below in next phase
!        !!MFW, 10/18/15: Will calculate burial based on the amount of deposition for the day
!        if (rchdy(57,jrch) < (bedvol * 1300000. / 1000000.)) then
!           bury = sedpstmass * (rchdy(57,jrch) / 
!     &     (bedvol * 1300000. / 1000000.))
!        else
!           bury = sedpstmass
!        end if
!        sedpstmass = sedpstmass - bury

        !!MFW, 10/15/15: Set settled pesicide to fraction of sediment that is deposited
        !!HR Sep 2019 activate code below in next phase
!        if (sedcon > 0.) then
!           setlpst = (frsrb * chpstmass) * (rchdy(57,jrch) /  
!     &            (sedinorg + rchdy(56,jrch))) !varoute(3,inum2) + sedst(jrch);   
!        else
!           setlpst = 0.
!        end if 
        ...
        !!MFW, 10/18/15: Modified for burial dependence on how much settled pesticide is added here
        !! If deposition is greater than the bed sediment capacity, only a fraction of the settled is added to active layer
        !!HR Sep 2019 activate code below in next phase
!        if (rchdy(57,jrch) > (bedvol * 1300000. / 1000000.)) then
!           sedpstmass = sedpstmass + (setlpst * 
!     &     ((bedvol * 1300000. / 1000000.) / rchdy(57,jrch)))
!        else
!           sedpstmass = sedpstmass + setlpst
!        end if
        ...
        ! HR Aug 2019; full day for in storage, actual time for routed
        !resuspst = chpst_rsp(jrch) * sedpstmass * tday / depth
        resuspst = chpst_rsp(jrch) * sedpstmass / depth * ((1.))   !!MW_2019: Since we are not routing sediment in benthic I think this process is 100% for the full 1-day timestep
        ...
        ! HR Aug 2019; full day for in storage, actual time for routed
        !difus = chpst_mix(jrch) * (fd2 * sedpstmass - frsol * chpstmass) * tday / depth
        !MW: Mods 10/21/19 ...  no new code implemented
        !MW: Assume chpst_mix(jrch) is equivalent to VVWM D/delta-x
        !MW: CT2 = sedpst_conc(jrch) = sedpstmass / bedvol = total benthic concentration (mg/m3)
        !MW: CT1 = chpst_conc(jrch) = chpstmass / (rchwtr + wtrin) = total water column concentration (mg/m3)
        !MW: K = CT2/CT1 = total partition coefficient
        !MW: A = benthic/water interface = ch_w(2,jrch) * ch_l2(jrch) * 1000. (m2)
        !MW: D = effective dispersion (m2/s) = 8.33E-09
        !MW: dx = boundary layer thinkness (m) = 1.02 (m) (VVWM defualt, could be varied by depth)
        !MW: c1 = water column aqueous phase concentration =  (frsol * chpstmass) / (rchwtr + wtrin)
        !MW: c2 = benthic aqueous phase concentration = (fd2 * sedpstmass)/ 

        difus = chpst_mix(jrch) * (fd2 * sedpstmass - frsol * chpstmass) / depth * ((tday * fr_routed) + (1. * fr_stored))   !!MW_2019: This makes sense to me
        if (difus > 0.) then
        ...
        !! by burial
        !!HR Sep 2019 remove code below in next phase
...
!! sediment processes
      ...
      !!reactb = sedpst_rea(jrch) * sedpstmass !HR this already assumes a day; OK all in storage
      !!MFW, 10/16/15: Original equation replaced it with 1st order decay
      reactb = sedpstmass - (sedpstmass * EXP(-1. * sedpst_rea(jrch) * 1.)) !! assume full one day for bentich sediment
...
!! calculate pesticide concentrations at end of day
      chpst_conc(jrch) = 0.
      sedpst_conc(jrch) = 0.
      if (rchwtr + wtrin > 1.e-9) then         !HR -rtwtr is a big factor !!MW: Yes, we shoudl not include -RTWTR ... the chpstmass still has not removed routed mass!!
        chpst_conc(jrch) = chpstmass / (rchwtr + wtrin)  !!MW_2019: I think this should be, chpst_conc(jrch) = chpstmass / (rchwtr + wtrin - rtwtr)
      else
...
!! calculate amount of pesticide transported out of reach
      if (rtwtr / 86400. > 1.e-9) then             !HR Aug 2019 - Change threshold from 0.002 to allow minor stream flow!Claire, corrected to match line 151
        solpesto = chpst_conc(jrch) * frsol      !!MW_2019: This is interesting that routed mass is based on end of time step concentration
        sorpesto = chpst_conc(jrch) * frsrb      !!MW_2019: Feel like we should be taking avg of beginning of time step and end of time step conc
      !!MW 2019: Would suggest the following
        !if (rchwtr / 86400. > 1.e-9) then
          !!solpesto = ((pstprev / rchwtr) + chpst_conc(jrch))/2. * frsol
          !!sorpesto = ((pstprev / rchwtr) + chpst_conc(jrch))/2  * frsrb
          !!MW: New Suggestion, redfined the initial concentration to include pesticide inflow
          !solpesto = ((pstprev / (rchwtr + wtrin)) + chpst_conc(jrch))/2. * frsol
          !sorpesto = ((pstprev / (rchwtr + wtrin)) + chpst_conc(jrch))/2  * frsrb
        !else
        !  solpesto = (0 + chpst_conc(jrch))/2. * frsol
        !  sorpesto = (0 + chpst_conc(jrch))/2  * frsrb
        !endif 
      else
        solpesto = 0.
        sorpesto = 0.
