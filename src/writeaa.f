...
!! calculate average annual values for watershed data
      wshdaao = wshdaao / yrs
      wpstaao = wpstaao !/ yrs HR print total instead of average annual
      ...
      wshd_pstap = wshd_pstap !HR total instead of by year / yrs
      wshd_pstdg = wshd_pstdg !HR total instead of by year / yrs
...
!! HR added lateral, tile, and groundwater contributions, and plant and soil
      if (iprp /= 0) then
        write (30,5500)
        do j = 1, nhru
          if (hrupest(j) == 1) then
                write (30,5600) subnum(j), hruno(j), yrs,               
     &                     (hrupsta(k,1,j), hrupsta(k,2,j),
     &                      hrupsta(k,4,j), hrupsta(k,5,j),
     &                      hrupsta(k,6,j), hrupsta(k,7,j),
     &                      hrupsta(k,8,j), hrupsta(k,9,j), k = 1, npmx)
          end if
        end do
      end if
...
!! write life span of septic HRUs (time to first failure, years)
 ...
 5600 format (1x,a5,a4,1x,f4.0,4x,1x,250(e17.4,1x))
