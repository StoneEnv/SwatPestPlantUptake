...
!!    sol_pst(:,:,:)|kg/ha        |amount of pesticide in soil profile
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &           (qtile + sol_prk(ly,j) + flat(ly,j) + 1.e-6)
                ...
                !! JK: we need to have csurf as an array for each pest and soil layer
                csurf = 0.
                if (ly == 1) then
                  csurf = percop * co             !Concentration of pesticde in surface, lateral, and tile runoff mgl/l  JK: 100mg/l
                else
                  csurf = co
                end if
                sol_pst_conc(k,j,ly) = csurf
            ...
            !if (curyr > nyskip .and. iprintpsthrulayer==1) then
              ...
              !write(*,*) subnum(j)
              !write(*,*) hruno(j)
              if (subnum(j)=='00006' .and. hruno(j)=='0092' .and. kk==236) then
     ...
     &             -1.
            ...
            !hendrik 12/2022 custom subsurface pest output
            iprintpstuptkhrulayer = 1
            if (curyr > 0 .and. iprintpstuptkhrulayer==1) then
            !if (curyr > nyskip .and. iprintpsthrulayer==1) then
              !if (j==2 .and. kk==236) then
              !write(*,*) subnum(j)
              !write(*,*) hruno(j)
              !if (subnum(j)=='00001' .and. hruno(j)=='0092' .and. kk==236) then
              if (subnum(j)=='00001' .and. hruno(j)=='0092') then
              !if (subnum(j)=='00006' .and. hruno(j)=='0092' .and. kk==236) then
               if (qtile > 1.e-06) then
                 tile_pst_conc = tile_pst(k)*100000/qtile
               else
               ...
               !write(*,*) iyr, iida, ly
               write(32138,"(2i11, 2a11, 3i11, 20e11.3)") iyr, iida,      
     &             subnum(j), hruno(j), kk, ly, ldrain(j),              
     &             sol_pst(k,j,ly),lat_pst_prnt, tile_pst(k),         
     &             xx,                                        
     &             qtile, flat(ly,j), sol_prk(ly,j), gw_q(j),pst_surq(k,j),
     &             revapday,gwseep,
     &             sol_pst_conc(k,j,ly),
     &             vf, sol_ul(ly,j), sol_kp(k,j,ly), 
     &             reg,
     &             dg,
     &             mobPest_frac,
     &             wuse(ly), plt_upst_ly(k,ly)
               if (ly==sol_nly(j)) then !print groundwater
                write(32138,"(2i11, 2a11, 3i11, 20e11.3)") iyr, iida,      
     &             subnum(j), hruno(j), kk, ly+1, 0,              
     &             pst_shallst(k,j),0, 0,         
     &             xx,                                        
     &             0, 0, sol_prk(ly,j), gw_q(j),0,
     &             revapday,gwseep,
     &             0,
     &             0, 0, 0, 0, 0,
     &             mobPest_frac,
