...
!     &             (sol_no3(j1,j), j1 = 1, sol_nly(j))
     ...
     &                      hrupstd(k,4,j), hrupstd(k,5,j),
     &                      hrupstd(k,6,j), hrupstd(k,7,j),
     &                      hrupstd(k,8,j), hrupstd(k,9,j), k = 1, npmx) !HR added lateral, tile, and gw, pest in soil, pest on plant
            end if
            if (iprp == 2) then !HR write concentrations in ug/L (convert from mg/mm); except for soil and plant mass
                do k=1, npmx
                if (surfq(j)>0) then
     ...
     &                     (conc1(k), conc2(k), conc3(k), conc4(k), conc5(k), conc6(k), 
     &                      hrupstd(k,8,j), hrupstd(k,9,j), k = 1, npmx)
            end if
          end if
          end if
        end do
